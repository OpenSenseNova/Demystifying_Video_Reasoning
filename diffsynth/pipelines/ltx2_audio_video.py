...
class LTX2AudioVideoPipeline(BasePipeline):
    ...
    def denoise_stage(self, inputs_shared, inputs_posi, inputs_nega, units, cfg_scale=1.0, progress_bar_cmd=tqdm, skip_stage=False,
                       step_callback=None, vis_steps=None, tiled=True, tile_size_in_pixels=512, tile_overlap_in_pixels=128, tile_size_in_frames=128, tile_overlap_in_frames=24):
        if skip_stage:
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            video_latents_before_step = inputs_shared["video_latents"]
            ...
            if step_callback is not None and (vis_steps is None or progress_id in vis_steps):
                ...
                x0_hat = self.scheduler.step(noise_pred_video, self.scheduler.timesteps[progress_id], video_latents_before_step, to_final=True)
                step_video = self.video_vae_decoder.decode(x0_hat, tiled, tile_size_in_pixels, tile_overlap_in_pixels, tile_size_in_frames, tile_overlap_in_frames)
    ...
    def __call__(
        ...
        inputs_shared, inputs_posi, inputs_nega = self.denoise_stage(
            ...
            step_callback=step_callback, vis_steps=vis_steps, tiled=tiled, tile_size_in_pixels=tile_size_in_pixels,
            tile_overlap_in_pixels=tile_overlap_in_pixels, tile_size_in_frames=tile_size_in_frames,
